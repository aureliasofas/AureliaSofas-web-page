...
document.addEventListener('DOMContentLoaded', () => {
    ...
    gsap.timeline()
        .to('.hero-content', { opacity: 1, y: 0, duration: 1, ease: 'power3.out', delay: 0.2 })
        .to('.hero-image', { scale: 1, duration: 2, ease: 'power2.out' }, "<");

    gsap.to('.feature-item', {
        scrollTrigger: { trigger: '.feature-bar', start: 'top 80%' },
    ...
    gsap.to('.section-header', {
        scrollTrigger: { trigger: '#collection', start: 'top 75%' },
        opacity: 1, y: 0, duration: 0.8
    });

    gsap.to(['.custom-content', '.custom-image'], {
        scrollTrigger: { trigger: '#custom', start: 'top 60%' },
        opacity: 1,
        x: 0,
        duration: 1,
        ease: 'power3.out',
    ...
    if (prevBtn && nextBtn) {
        prevBtn.addEventListener('click', () => scroller.scrollBy({ left: -400, behavior: 'smooth' }));
        nextBtn.addEventListener('click', () => scroller.scrollBy({ left: 400, behavior: 'smooth' }));
    }
